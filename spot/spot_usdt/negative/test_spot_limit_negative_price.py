...
@allure.story("Spot Limit Buy: Negative Price Validation")
@pytest.mark.spot
def test_spot_limit_negative_price():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with price = -100 and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTC/USDT"
    
    with allure.step("Step 1: Frame LIMIT order payload with price = -100"):
