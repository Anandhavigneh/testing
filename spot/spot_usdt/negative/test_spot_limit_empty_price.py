...
`price` field is an empty string ("").
...
@allure.story("Spot Limit Buy: Empty Price Validation")
@pytest.mark.spot
def test_spot_limit_empty_price():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with an empty price ("") and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTC/USDT"
    
    with allure.step("Step 1: Frame LIMIT order payload with price = \"\""):
