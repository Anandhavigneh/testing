...
`symbol` format is invalid (e.g., missing the forward slash delimiter).
...
@allure.story("Spot Limit Buy: Invalid Symbol Format Validation")
@pytest.mark.spot
def test_spot_limit_invalid_symbol_format():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with symbol = "BTCUSDT" and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTCUSDT"
    
    with allure.step("Step 1: Frame LIMIT order payload with invalid symbol = 'BTCUSDT'"):
        # We prepare standard valid parameters, except symbol
        order_side = "BUY"
        quantity = "0.001"
        price = "60000.0"
    ...
    with allure.step("Step 4: Verify proper error message and status"):
        ...
        assert "symbol" in str(msg).lower(), f"Error message should mention 'symbol'. Got: {msg}"
