...
`quantity` field is missing or set to `None`.
...
@allure.story("Spot Limit Buy: Missing Quantity Validation")
@pytest.mark.spot
def test_spot_limit_missing_quantity():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with quantity = None and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTC/USDT"
    
    with allure.step("Step 1: Frame LIMIT order payload with quantity = None"):
