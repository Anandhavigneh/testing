...
@allure.story("Spot Limit Buy: Zero Price Validation")
@pytest.mark.spot
def test_spot_limit_zero_price():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with price = 0 and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTC/USDT"
    
    with allure.step("Step 1: Frame LIMIT order payload with price = 0"):
