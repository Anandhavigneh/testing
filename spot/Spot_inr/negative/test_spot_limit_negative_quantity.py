...
@allure.story("Spot Limit Buy: Negative Quantity Validation")
@pytest.mark.spot
def test_spot_limit_negative_quantity():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with quantity = -0.10 and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTC/INR"
    
    with allure.step("Step 1: Frame LIMIT order payload with quantity = -0.10"):
