...
`price` field is missing or set to `None`.
...
@allure.story("Spot Limit Buy: Missing Price Validation")
@pytest.mark.spot
def test_spot_limit_missing_price():
    """
    Test Spot Limit Buy Negative Flow:
    Attempt to place a LIMIT order with price = None and verify rejection.
    """
    spot_client = SpotClient()
    symbol = "BTC/INR"
    
    with allure.step("Step 1: Frame LIMIT order payload with price = None"):
    ...
    with allure.step("Step 2: Send Spot LIMIT order request"):
        # This will either be caught by the SpotClient local validation or the Backend API.
        # Both are valid forms of 'rejection' for the framework.
    ...
    with allure.step("Step 4: Verify proper error message and status"):
        ...
        assert "price" in str(msg).lower(), f"Error message should mention 'price'. Got: {msg}"
