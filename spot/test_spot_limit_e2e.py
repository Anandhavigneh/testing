...
def test_limit_buy_by_valid_price_quantity(spot_client):
    ...
    assert result.get("Status") == "Success", (
        f"Real Limit BUY order FAILED — API response: {result}"
    )
...
def test_limit_sell_by_valid_price_quantity(spot_client):
    ...
    assert result.get("Status") == "Success", (
        f"Real Limit SELL order FAILED — API response: {result}"
    )
...
def test_limit_buy_using_affordability(spot_client):
    ...
    assert result.get("Status") == "Success", (
        f"Real Limit BUY (affordability) order FAILED — API response: {result}"
    )
