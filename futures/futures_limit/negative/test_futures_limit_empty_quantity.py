"""
================================================================================
        FUTURES LIMIT E2E TEST SUITE - EMPTY QUANTITY
================================================================================
...
│ NEGATIVE TEST                                                               │
├─────────────────────────────────────────────────────────────────────────────┤
│ • test_futures_limit_empty_quantity()                                                   │
│   → Empty quantity                                                    │
└─────────────────────────────────────────────────────────────────────────────┘
...
Example Execution:
  pytest futures_limit/negative/test_futures_limit_empty_quantity.py -v
  pytest futures_limit/negative/test_futures_limit_empty_quantity.py --alluredir=allure-results
...
@allure.story("Empty quantity")
@pytest.mark.serial
def test_futures_limit_empty_quantity(setup_data):
    client = setup_data["client"]
    with allure.step("Send order with empty quantity string"):
        resp = client.create_order(
            ctid=Config.DEFAULT_CTID, symbol=setup_data["symbol"], 
            qty="", price=setup_data["valid_price"], 
