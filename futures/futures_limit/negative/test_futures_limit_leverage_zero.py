"""
================================================================================
        FUTURES LIMIT E2E TEST SUITE - INVALID LEVERAGE (0)
================================================================================
...
│ NEGATIVE TEST                                                               │
├─────────────────────────────────────────────────────────────────────────────┤
│ • test_future_limit_leverage_zero()                                                   │
│   → Invalid leverage (0)                                                    │
└─────────────────────────────────────────────────────────────────────────────┘
...
Example Execution:
  pytest futures_limit/negative/test_futures_limit_leverage_zero.py -v
  pytest futures_limit/negative/test_futures_limit_leverage_zero.py --alluredir=allure-results
...
@allure.story("Invalid leverage (0)")
@pytest.mark.serial
def test_future_limit_leverage_zero(setup_data):
    client = setup_data["client"]
    with allure.step("Send order with leverage = 0"):
        resp = client.create_order(
            ctid=Config.DEFAULT_CTID, symbol=setup_data["symbol"], 
            qty=setup_data["valid_qty"], price=setup_data["valid_price"], 
            amount=setup_data["valid_amount"], order_type="1", order_side="0",
            leverage="0"
        )
