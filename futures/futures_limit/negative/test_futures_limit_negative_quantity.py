"""
================================================================================
        FUTURES LIMIT E2E TEST SUITE - NEGATIVE QUANTITY
================================================================================
...
│ NEGATIVE TEST                                                               │
├─────────────────────────────────────────────────────────────────────────────┤
│ • test_futures_limit_negative_quantity()                                                   │
│   → Negative quantity                                                    │
└─────────────────────────────────────────────────────────────────────────────┘
...
Example Execution:
  pytest futures_limit/negative/test_futures_limit_negative_quantity.py -v
  pytest futures_limit/negative/test_futures_limit_negative_quantity.py --alluredir=allure-results
...
@pytest.mark.serial
def test_futures_limit_negative_quantity(setup_data):
    client = setup_data["client"]
    with allure.step("Send order with negative quantity (-0.1)"):
        resp = client.create_order(
            ctid=Config.DEFAULT_CTID, symbol=setup_data["symbol"], 
            qty="-0.1", price=setup_data["valid_price"], 
