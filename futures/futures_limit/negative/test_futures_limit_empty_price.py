"""
================================================================================
        FUTURES LIMIT E2E TEST SUITE - EMPTY PRICE
================================================================================
...
│ NEGATIVE TEST                                                               │
├─────────────────────────────────────────────────────────────────────────────┤
│ • test_futures_limit_empty_price()                                                   │
│   → Empty price                                                    │
└─────────────────────────────────────────────────────────────────────────────┘
...
Example Execution:
  pytest futures_limit/negative/test_futures_limit_empty_price.py -v
  pytest futures_limit/negative/test_futures_limit_empty_price.py --alluredir=allure-results
...
@allure.story("Empty price")
@pytest.mark.serial
def test_futures_limit_empty_price(setup_data):
    client = setup_data["client"]
    with allure.step("Send order with empty price string"):
        resp = client.create_order(
            ctid=Config.DEFAULT_CTID, symbol=setup_data["symbol"], 
            qty=setup_data["valid_qty"], price="", 
