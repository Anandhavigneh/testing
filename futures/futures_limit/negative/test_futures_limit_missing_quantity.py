"""
================================================================================
        FUTURES LIMIT E2E TEST SUITE - MISSING QUANTITY
================================================================================
...
│ NEGATIVE TEST                                                               │
├─────────────────────────────────────────────────────────────────────────────┤
│ • test_futures_limit_missing_quantity()                                                   │
│   → Missing quantity                                                    │
└─────────────────────────────────────────────────────────────────────────────┘
...
Example Execution:
  pytest futures_limit/negative/test_futures_limit_missing_quantity.py -v
  pytest futures_limit/negative/test_futures_limit_missing_quantity.py --alluredir=allure-results
...
@allure.story("Missing quantity")
@pytest.mark.serial
def test_futures_limit_missing_quantity(setup_data):
    client = setup_data["client"]
    with allure.step("Send order with missing quantity (None)"):
        resp = client.create_order(
            ctid=Config.DEFAULT_CTID, symbol=setup_data["symbol"], 
            qty=None, price=setup_data["valid_price"], 
