"""
================================================================================
        FUTURES LIMIT E2E TEST SUITE - SHORT LEVERAGE
================================================================================
...
│ MAIN FLOW TESTS                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│ • test_futures_limit_short_leverage                                          │
└─────────────────────────────────────────────────────────────────────────────┘
...
Example Execution:
  pytest futures_limit/positive/test_futures_limit_short_leverage.py -v
  pytest futures_limit/positive/test_futures_limit_short_leverage.py --alluredir=allure-results
...
@allure.story("Futures Limit Orders: Adjust Leverage, Take Trade, and Place TP/SL on Short Position")
@pytest.mark.serial
def test_futures_limit_short_leverage():
    """
    Test complete flow: Set leverage explicitly, fetch config, place a short limit order,
    verify the active position's leverage matches the set value, and apply TP/SL targets.
    """
    futures_client = FuturesTradingClient()
    symbol = "BTC/USDT"
    
    # We use 2X leverage to fulfill the verification requirement in the prompt
    target_leverage = 2
    ...
    with allure.step("Step 2: Fetch Order Book and Get Current Bid Price"):
        ...
        top_bid = float(btc_ticker['top_bid'])
    ...
    # (Crossing the spread instead of resting in orderbook)
    limit_price = top_bid - 5.0
    price_str = f"{limit_price:.1f}"
    amount_str = f"{qty_val * limit_price:.4f}"

    with allure.step("Step 4: Frame Payload and Take Trade (Short Limit Order)"):
    ...
    with allure.step("Step 6: Verify Active Position Opened with Correct Leverage"):
        ...
        assert len(btc_short_positions) > 0, f"{symbol} SHORT position not found in active positions."
        recent_pos = btc_short_positions[0]

        # Verify Leverage
        actual_leverage = recent_pos.get("leverage", "")
        # The API may return '2' or '2x'
        ...
        # Use abs() because Giottus might represent short quantity as negative
        assert abs(pos_qty) > 0, f"Position quantity is not greater than 0: {pos_qty}"
        assert entry_price > 0, "Entry price is invalid / 0"
        
        abs_qty_str = str(abs(pos_qty))

    # ===== PHASE 5: Place TP/SL =====
    with allure.step("Step 7: Place TP / SL on Active Position"):
        # Calculate TP and SL based on the actual entry price!
        # Giottus requires SL to be at least ~3% above entry for SHORT.
